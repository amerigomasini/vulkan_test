...
private:
	...
	void createGraphicsPipeline()
	{
		...
		bindingDescription.stride = sizeof(VertexDI);
		vertexInputBindingDescriptions.push_back(bindingDescription);

		bindingDescription.binding = currentBinding++;
		bindingDescription.stride = sizeof(ColorDI);
		vertexInputBindingDescriptions.push_back(bindingDescription);

		bindingDescription.binding = currentBinding++;
		bindingDescription.stride = sizeof(TextureCoordinateDI);
		vertexInputBindingDescriptions.push_back(bindingDescription);

		bindingDescription.binding = currentBinding++;
		bindingDescription.stride = sizeof(NormalDI);
		vertexInputBindingDescriptions.push_back(bindingDescription);
		...
		vertexInputInfo.vertexAttributeDescriptionCount = static_cast<uint32_t>(vertexInputAttributeDescriptions.size());	//Descriptors of data passed to Vertex Shader
	...
	void createBuffer(VkDeviceSize size, VkBufferUsageFlags vkUsage, VmaMemoryUsage memoryUsage, VkMemoryPropertyFlags properties, VkBuffer & buffer, VmaAllocation & allocation)
	{
		//TODO
		...
		bufferInfo.usage = vkUsage;									//how will we use this generic buffer? multiple flags can be ORd together
	...
	void createVertexBuffers()
	{
		...
		createBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VMA_MEMORY_USAGE_CPU_ONLY, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, stagingBuffer, stagingAllocation);
		...
		bufferSize = sizeof(colors[0]) * 8;
		createBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VMA_MEMORY_USAGE_CPU_ONLY, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, stagingBuffer, stagingAllocation);

		vmaMapMemory(allocator, stagingAllocation, &data);
		memcpy(data, colors, bufferSize);
		vmaUnmapMemory(allocator, stagingAllocation);

		//cannot map memory which is local to the device, instead we can copy buffers, if they were created with transfer flags
		createBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VMA_MEMORY_USAGE_GPU_ONLY, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, colorBuffer, colorBufferAllocation);
		copyBuffer(stagingBuffer, colorBuffer, bufferSize);
		vmaDestroyBuffer(allocator, stagingBuffer, stagingAllocation);



		bufferSize = sizeof(texs[0]) * 8;
		createBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VMA_MEMORY_USAGE_CPU_ONLY, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, stagingBuffer, stagingAllocation);

		vmaMapMemory(allocator, stagingAllocation, &data);
		memcpy(data, texs, bufferSize);
		vmaUnmapMemory(allocator, stagingAllocation);

		//cannot map memory which is local to the device, instead we can copy buffers, if they were created with transfer flags
		createBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VMA_MEMORY_USAGE_GPU_ONLY, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, texBuffer, texBufferAllocation);
		copyBuffer(stagingBuffer, texBuffer, bufferSize);
		vmaDestroyBuffer(allocator, stagingBuffer, stagingAllocation);



		bufferSize = sizeof(normals[0]) * 8;
		createBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VMA_MEMORY_USAGE_CPU_ONLY, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, stagingBuffer, stagingAllocation);

		vmaMapMemory(allocator, stagingAllocation, &data);
		memcpy(data, normals, bufferSize);
		vmaUnmapMemory(allocator, stagingAllocation);

		//cannot map memory which is local to the device, instead we can copy buffers, if they were created with transfer flags
		createBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VMA_MEMORY_USAGE_GPU_ONLY, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, normalBuffer, normalBufferAllocation);
	...
	void createImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling,
		VkImageUsageFlags usage, VmaMemoryUsage memoryUsage, VkMemoryPropertyFlags properties, VkImage & image, VmaAllocation & imageMemory)
	{
		...
		allocInfo.requiredFlags = properties;
		allocInfo.usage = memoryUsage;
		VkResult result = vmaCreateImage(allocator, &imageInfo, &allocInfo, &image, &imageMemory, nullptr);
		if (result != VK_SUCCESS)
	...
	void createTextureImage()
	{
		...
		createImage(texWidth, texHeight, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_TILING_OPTIMAL,
			VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VMA_MEMORY_USAGE_GPU_ONLY, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
	...
	void createCommandBuffers()
	{
		...
		for (size_t i = 0; i < commandBuffers.size(); ++i)
		{
			...
			VkBuffer vertexBuffers[] = { vertexBuffer, colorBuffer, texBuffer, normalBuffer };
			VkDeviceSize offsets[] = { 0, 0, 0, 0 };
			vkCmdBindVertexBuffers(commandBuffers[i], 0, 4, vertexBuffers, offsets);
	...
	void createDepthResources()
	{
		...
		createImage(swapChainExtent.width, swapChainExtent.height, depthFormat, VK_IMAGE_TILING_OPTIMAL,
			VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VMA_MEMORY_USAGE_GPU_ONLY, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, depthImage, depthImageMemory);
	...
	void cleanup()
	{
		...
		vmaDestroyBuffer(allocator, colorBuffer, colorBufferAllocation);
		vmaDestroyBuffer(allocator, texBuffer, texBufferAllocation);
		vmaDestroyBuffer(allocator, normalBuffer, normalBufferAllocation);
