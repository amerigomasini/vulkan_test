...
private:
	...
	void cleanupSwapChain()
	{
		...
		vkDestroyPipeline(device, noTexturePipeline, nullptr);
	...
	void createDescriptorSetLayout()
	{
		...
		uboLayoutBindings[0].binding = 0;											//binding used in the shader
		uboLayoutBindings[0].descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;	//type of descriptor
		uboLayoutBindings[0].descriptorCount = 1;
		uboLayoutBindings[0].stageFlags = VK_SHADER_STAGE_VERTEX_BIT;				//which shader stage will use it. different flags can be ORd together
		uboLayoutBindings[0].pImmutableSamplers = nullptr;							//only relevant for image sampling
	...
	void createGraphicsPipeline()
	{
		...
		*/
		...
		specializationMapEntry.offset = offsetof(SpecializationConstants, useTextures);
		specializationMapEntry.size = sizeof(specializationConstants.useTextures);
		...
		//create no-texture pipeline
		specializationConstants.useTextures = false;
		if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &noTexturePipeline) != VK_SUCCESS)
			throw std::runtime_error("failed to create no-texture pipeline");
	}
	...
	void createCommandBuffers()
	{
		...
		for (size_t i = 0; i < commandBuffers.size(); ++i)
		{
			...
			vkCmdSetViewport(commandBuffers[i], 0, 1, &viewport);
			vkCmdBindPipeline(commandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, noTexturePipeline);
