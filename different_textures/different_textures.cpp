...
private:
	...
	void createTextureImage(std::string const & image_file_name, VkImage & outImage, VkDeviceMemory & outImageMemory)
	{
		//TODO
		...
		stbi_uc * pixels = stbi_load(image_file_name.c_str(), &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
	...
	void updateUniformBuffer(uint32_t currentImage)
	{
		...
		for (auto const & oneModel : models)
		{
			...
			if (oneModel.index == 1)
				ubo.model = glm::translate(ubo.model, glm::vec3(1.7f, 0.0f, 0.0f));
