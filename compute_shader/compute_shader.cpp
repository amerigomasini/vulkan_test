...
std::vector<uint32_t> indices = Utilities::getSquareIndices();
...
private:
	...
	//each model will have its own uniform buffers and descriptor sets
	struct Model
	{
	public:
	...
	struct QueueFamilyIndices
	{
		...
		std::optional<uint32_t> computeFamily;		//Queue capable of computing
	...
	QueueFamilyIndices findQueueFamilies(VkPhysicalDevice device)
	{
		...
		{
			//try to find a queue that ONLY supports computing
	...
	void createDescriptorSetLayout()
	{
		...
		layoutBindings[0].binding = 0;											//binding used in the shader
		layoutBindings[0].descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;	//type of descriptor
		layoutBindings[0].descriptorCount = 1;
		layoutBindings[0].stageFlags = VK_SHADER_STAGE_VERTEX_BIT;				//which shader stage will use it. different flags can be ORd together
		layoutBindings[0].pImmutableSamplers = nullptr;							//only relevant for image sampling

		layoutBindings[1].binding = 1;											//binding used in the shader
		layoutBindings[1].descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;	//type of descriptor
		layoutBindings[1].descriptorCount = 1;
		layoutBindings[1].stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT;				//which shader stage will use it. different flags can be ORd together
		layoutBindings[1].pImmutableSamplers = nullptr;							//only relevant for image sampling
	...
	void createComputePipeline()
	{
		...
		//0th binding will be the image we read from
		VkDescriptorSetLayoutBinding oneBinding;
		oneBinding.binding = 0;
		oneBinding.descriptorCount = 1;
		oneBinding.descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_IMAGE;
		oneBinding.stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
		bindings.push_back(oneBinding);

		//1st binding will be the image we write to
		oneBinding.binding = 1;
		bindings.push_back(oneBinding);
		...
		if (vkCreateDescriptorSetLayout(device, &descriptorSetLayoutInfo, nullptr, &computeDescriptorSetLayout) != VK_SUCCESS)
		...
		VkPipelineShaderStageCreateInfo shaderStageInfo = loadShader("shaders/edge_detection_comp.spv", VK_SHADER_STAGE_COMPUTE_BIT);
		...
		if (vkCreateComputePipelines(device, nullptr, 1, &computePipelineInfo, nullptr, &computePipeline) != VK_SUCCESS)
	...
	void createComputeTextureTarget(Model & model)
	{
		//query for supported format. we want to use the same format we used for the source texture
		VkFormatProperties properties;
		vkGetPhysicalDeviceFormatProperties(physicalDevice, VK_FORMAT_R8G8B8A8_UNORM, &properties);

		//it needs to support image storage operations
		assert(properties.optimalTilingFeatures & VK_FORMAT_FEATURE_STORAGE_IMAGE_BIT);

		//create the image
		createImage(model.texWidth, model.texHeight, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_TILING_OPTIMAL,
		...
		createTextureImageView(model.computeImage, model.computeImageView);
	}
	...
	void createTextureImage(std::string const & image_file_name, VkImage & outImage, VkDeviceMemory & outImageMemory, int & outTexWidth, int & outTexHeight)
	{
		//TODO
		...
		int texChannels;
		stbi_uc * pixels = stbi_load(image_file_name.c_str(), &outTexWidth, &outTexHeight, &texChannels, STBI_rgb_alpha);
		VkDeviceSize imageSize = outTexWidth * outTexHeight * 4;
		...
		createImage(outTexWidth, outTexHeight, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_TILING_OPTIMAL,
			VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
	...
	void createTextureImageView(VkImage const & textureImage, VkImageView & outImageView)
	{
		outImageView = createImageView(textureImage, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_ASPECT_COLOR_BIT);
	}
	...
	void createCommandBuffers()
	{
		...
		for (size_t i = 0; i < commandBuffers.size(); ++i)
		{
			...
			//left image
			vkCmdBindDescriptorSets(commandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 0, 1, &models[0].descriptorSet, 0, nullptr);
			vkCmdDrawIndexed(commandBuffers[i], static_cast<uint32_t>(indices.size()), 1, 0, 0, 0);

			//right image, after processing
			viewport.x += viewport.width;
			vkCmdSetViewport(commandBuffers[i], 0, 1, &viewport);
			vkCmdBindDescriptorSets(commandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 0, 1, &models[0].postProcessingDescriptorSet, 0, nullptr);
			vkCmdDrawIndexed(commandBuffers[i], static_cast<uint32_t>(indices.size()), 1, 0, 0, 0);
	...
	void createComputeCommandBuffer()
	{
		...
		vkAllocateCommandBuffers(device, &commandBufferAllocInfo, &computeCommandBuffer);


		//assign instructions to the command buffer
		...
		vkCmdDispatch(computeCommandBuffer, models[0].texWidth / 16, models[0].texHeight / 16, 1);
	...
	void createDescriptorSets(Model & model)
	{
		...
		if (vkAllocateDescriptorSets(device, &allocInfo, &model.postProcessingDescriptorSet) != VK_SUCCESS)
			throw std::runtime_error("failed to create descriptor sets");

		std::array<VkWriteDescriptorSet, 2> postDescriptorWrites = {};

		imageInfo.sampler = model.computeSampler;
		imageInfo.imageView = model.computeImageView;

		postDescriptorWrites[0].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
		postDescriptorWrites[0].descriptorCount = 1;
		postDescriptorWrites[0].descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
		postDescriptorWrites[0].dstBinding = 0;				//the shader will access this using binding = 0
		postDescriptorWrites[0].dstSet = model.postProcessingDescriptorSet;
		postDescriptorWrites[0].dstArrayElement = 0;
		postDescriptorWrites[0].pBufferInfo = &bufferInfo;	//only relevant for descriptors that refer to buffers
		postDescriptorWrites[0].pImageInfo = nullptr;		//only relevant for descriptors that refer to images
		postDescriptorWrites[0].pTexelBufferView = nullptr;	//only relevant for descriptors that refer to buffer views

		postDescriptorWrites[1].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
		postDescriptorWrites[1].descriptorCount = 1;
		postDescriptorWrites[1].descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
		postDescriptorWrites[1].dstBinding = 1;				//the shader will access this using binding = 1
		postDescriptorWrites[1].dstSet = model.postProcessingDescriptorSet;
		postDescriptorWrites[1].dstArrayElement = 0;
		postDescriptorWrites[1].pBufferInfo = nullptr;		//only relevant for descriptors that refer to buffers
		postDescriptorWrites[1].pImageInfo = &imageInfo;	//only relevant for descriptors that refer to images
		postDescriptorWrites[1].pTexelBufferView = nullptr;	//only relevant for descriptors that refer to buffer views

		vkUpdateDescriptorSets(device, static_cast<uint32_t>(postDescriptorWrites.size()), postDescriptorWrites.data(), 0, nullptr);
	}
	...
	void createComputeDescriptorSets()
	{
		...
		if (vkAllocateDescriptorSets(device, &descriptorSetAllocInfo, &computeDescriptorSet) != VK_SUCCESS)
		...
		oneDescriptor.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
		oneDescriptor.descriptorCount = 1;
		oneDescriptor.descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_IMAGE;
		oneDescriptor.dstBinding = 0;
		oneDescriptor.dstSet = computeDescriptorSet;
		oneDescriptor.dstArrayElement = 0;
		oneDescriptor.pBufferInfo = nullptr;
		oneDescriptor.pTexelBufferView = nullptr;
		oneDescriptor.pImageInfo = &imageInfo;

		std::vector<VkWriteDescriptorSet> descriptorSets;
		descriptorSets.push_back(oneDescriptor);

		//binding 1 is the destination image
	...
	void initVulkan()
	{
		...
		createDepthResources();
		createFramebuffers();
		models[0].textureName = "../common/textures/zoidberg.jpg";
		for (auto & oneModel : models)
		{
			createTextureImage(oneModel.textureName, oneModel.textureImage, oneModel.textureImageMemory, oneModel.texWidth, oneModel.texHeight);
			...
			//create texture image which will hold the result of the compute shader
			createComputeTextureTarget(oneModel);
		}
	...
	void updateUniformBuffer()
	{
		...
		for (auto const & oneModel : models)
		{
			...
			ubo.view = glm::lookAt(glm::vec3(0, 0, 2), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, -1.0f, 0.0f));
			ubo.proj = glm::perspective(glm::radians(45.0f), ((float)swapChainExtent.width / 2.0f) / (float)swapChainExtent.height, 0.1f, 100.0f);
	...
	void drawFrame()
	{
		...
		if (vkQueueSubmit(computeQueue, 1, &computeSubmitInfo, computeFence) != VK_SUCCESS)
			throw std::runtime_error("failed to submit compute request");
	...
	void cleanup()
	{
		...
		//do not think this pipeline depends on the window size
		...
		for (auto & oneModel : models)
		{
			vkDestroySampler(device, oneModel.textureSampler, nullptr);
			vkDestroyImageView(device, oneModel.textureImageView, nullptr);
			...
			vkDestroyImage(device, oneModel.computeImage, nullptr);
			vkFreeMemory(device, oneModel.computeImageMemory, nullptr);
			vkDestroyImageView(device, oneModel.computeImageView, nullptr);
			vkDestroySampler(device, oneModel.computeSampler, nullptr);
		}
