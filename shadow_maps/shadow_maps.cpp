...
const std::string MODEL_PATH = "../common/models/samplescene.dae";
...
private:
	...
	void createOffscreenRenderPass()
	{
		...
		depthAttachmentReference.layout = VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL;	//during the render pass it is used as a depth buffer
		...
		endDependency.srcStageMask = VK_PIPELINE_STAGE_LATE_FRAGMENT_TESTS_BIT;
		...
		renderPassInfo.pDependencies = dependencies.data();		//dependencies are used for layout transitions
		renderPassInfo.subpassCount = 1;
	...
	void initVulkan()
	{
		...
		Utilities::loadModel(MODEL_PATH, vertices, indices, meshes);
		createVertexBuffers();
		createIndexBuffers();
	...
	glm::vec3 position = glm::vec3(0, -10, 40);
	...
	void updateUniformBuffer(uint32_t)
	{
		...
		horizontalAngle += -1.0f * mouseSpeed * float(swapChainExtent.width / 2 - xpos);
		verticalAngle += -1.0f * mouseSpeed * float(swapChainExtent.height / 2 - ypos);
